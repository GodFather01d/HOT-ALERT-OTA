  ...
  void sendhistory(String Path)
  {
   // Serial.println("im here");
    ...
    {
      Firebase.setInt(fbdo, "SYSTEM/" + String(sys_id) + "/"+ Path +"/HUMD", ALERT_HUMD);  
     ...
     if(Serial.available()) 
      {
       ...
       if (strcmp(inputCharArray, "JT_SIREN_OFF") == 0) 
         ...
         if (strcmp(inputCharArray, "JT_BATOK") == 0) 
        {
          ...
          if (strcmp(inputCharArray, "JT_ACON") == 0) 
        {
           Firebase.setInt(fbdo, "SYSTEM/" + String(sys_id) + "/ON_OFF_STATE", 1);
        }
         if (strcmp(inputCharArray, "JT_ACOFF") == 0) 
        {
    ...
    if (WiFi.status() == WL_CONNECTED) 
      {
         ...
         if (Firebase.getInt(fbdo,"SYSTEM/" + String(sys_id) + "/BIT_SET/SET_BUTTON"))
        {
          ...
          if(value == 1)
          {
            Firebase.getString(fbdo,"SYSTEM/" + String(sys_id) + "/BIT_SET/VALUE");
            String bit = fbdo.stringData();
            String set_bit = "JT_BIT" + bit;
