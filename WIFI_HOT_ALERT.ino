  ...
  WiFiManager wm;
  unsigned char update_ota = 0,webupdate_flag = 0,JT_UPDATE_UNSUCCESS_FLAG = 0,ALERT_SENSOR = 0,doorNumber = 0;
  unsigned char senddoorhistory_flag = 0,sendtemphistory_flag = 0, sendhumdhistory_flag = 0, sendsmokehistory_flag = 0,sendwaterhistory_flag = 0;
    ...
    if (WiFi.status() == WL_CONNECTED) 
      {
         ...
         if (Firebase.getInt(fbdo,"SYSTEM/" + String(sys_id) + "/SET_TEMP_HUMD/SET_BUTTON"))
        {
          ...
          if(val == 1)
          {
            ...
            Firebase.setInt(fbdo, "SYSTEM/" + String(sys_id) + "/NOTIFICATION/SET_VALUES/MIN", SET_TEMP);
            Firebase.setInt(fbdo, "SYSTEM/" + String(sys_id) + "/NOTIFICATION/SET_VALUES/MIN", SET_HUMD);
